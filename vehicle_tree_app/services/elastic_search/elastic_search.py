...
from vehicle_tree_app.services.elastic_search.indexing.user_indexing import UserIndexConfig
...
class SearchELK(UserIndexConfig, Generic[ElasticPydanticModel]):
    ...
    def create_index(self):
        ...
        try:
            self.es.indices.create(index=self.es_index_name, mappings=self.es_index_mapping, settings=self.es_settings,
                                   ignore=400)
        except Exception as e:
            print(f"Error creating index: {e}")

    @index_check_decorator
    def create_doc(self, data: Dict[str, Any]):
        """Creates a document in the Elasticsearch index."""
        return self.es.index(index=self.es_index_name, document=data)

    def get_doc(self, index_name: str, query: Dict[str, Any]):
        """Fetches a document from Elasticsearch."""
    ...
    def check_index_exists(self) -> bool:
        ...
        """
        return self.es.indices.exists(index=self.es_index_name)
    ...
    def index(self) -> Dict:
        ...
        return self.es.indices.get(index=self.es_index_name)
    ...
    def search(self, query: dict | None = None):
        return self.es.search(index=self.es_index_name, body=query)
...
#
#         return self.es.search(index=self.read_name, body=query)
# #
